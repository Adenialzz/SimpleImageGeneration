...
def generate(
    ...
    with torch.no_grad():
        ...
        tokenizer = Tokenizer()
        clip = models.get('clip') or util.load_clip(device)
        clip.to(device)
        if do_cfg:
        ...
        if input_images:
            encoder = models.get('encoder') or util.load_encoder(device)
            encoder.to(device)
        ...
        diffusion = models.get('diffusion') or util.load_diffusion(device)
        diffusion.to(device)
        ...
        decoder = models.get('decoder') or util.load_decoder(device)
        decoder.to(device)
